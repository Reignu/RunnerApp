...
CREATE PROCEDURE `AddRunningComp` (IN `inSeason` TEXT, IN `inCompetition` TEXT, IN `inVenue` TEXT, IN `inRank` INT)
BEGIN
    INSERT INTO RUNNINGCOMP (SEASON, COMPETITION, VENUE, RANK)
    VALUES (inSeason, inCompetition, inVenue, inRank);
END //

CREATE PROCEDURE `AddRunner` (IN `inRunnerNumber` TEXT, IN `inRunnerName` TEXT, IN `inGender` VARCHAR(1), IN `inRunningCompId` INT)
BEGIN
...
CREATE PROCEDURE `GetRunnerComp` (IN `inRunningCompId` INT)
BEGIN
    SELECT *
    FROM RUNNINGCOMP
...
CREATE PROCEDURE `GetRunnerComps` ()
BEGIN
    SELECT RUNNINGCOMPID, SEASON, COMPETITION, VENUE, RANK 
    FROM RUNNINGCOMP 
    Order By RUNNINGCOMPID;
END //

CREATE PROCEDURE `GetRunningCompRunners` (IN `inRunningCompId` INT)
BEGIN
    SELECT RUNNERID, RUNNERNUMBER, RUNNERNAME, GENDER 
    FROM RUNNER
    WHERE RUNNINGCOMPID = inRunningCompId
